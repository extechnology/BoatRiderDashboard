...
export function ExportToolbar({ data, filename, dateField, showFilter = true }: ExportToolbarProps) {
  const [filter, setFilter] = useState<DateFilter | "all">("all");
  const [customDate, setCustomDate] = useState("");

  const handleExport = () => {
    const filtered = filter === "all" ? data : filterByDate(data as any[], filter, dateField, customDate);
